...
import { _removeUser } from '@/pages/setting/user/user.service.ts';
...
import { OrgUserPageDTO } from '@/pages/setting/org/org.types.ts';
import {
  _getOrgUserList,
  _removeUserFromOrg
} from '@/pages/setting/org/org.service.ts';
import OrgUserOperate from '@/pages/setting/org/org-user-operate/OrgUserOperate.vue';

const loading = ref(false);
const { openDialog } = useDialog();
const queryData = ref<OrgUserPageDTO>({
  pageNum: 1,
  pageSize: 10,
  total: 0
});
...
watch(
  () => props.orgId,
  () => {
    queryData.value.orgId = props.orgId;
    init();
  }
);
...
const init = () => {
  loading.value = true;
  _getOrgUserList(queryData.value).then((res) => {
    if (res.code === ResponseCode.SUCCESS) {
